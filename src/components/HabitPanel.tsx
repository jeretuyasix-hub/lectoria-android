...
export default function HabitPanel({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved?: (settings: HabitSettings) => void }) {
  ...
  return <AnimatePresence>{open && <motion.aside role="dialog" aria-modal="true" aria-label="Metas y recordatorios" tabIndex={-1} className="habit-panel side-panel" initial={{ x: '105%' }} animate={{ x: 0 }} exit={{ x: '105%' }} transition={{ type: 'spring', stiffness: 340, damping: 34 }}>
    <header className="panel-header"><div><div className="eyebrow">HÁBITO DE LECTURA</div><h2>Metas y recordatorios</h2><p>Incentivos discretos: sin convertir la lectura en una colección de puntos.</p></div><button onClick={onClose} aria-label="Cerrar hábitos">×</button></header>
    <div className="habit-content">
      ...
      <section className="habit-card"><h3>Meta diaria</h3><div className="number-setting"><input aria-label="Minutos de meta diaria" type="number" min="5" max="240" value={settings.dailyGoalMinutes} onChange={e => setSettings(s => ({ ...s, dailyGoalMinutes: Number(e.target.value) }))}/><span>minutos al día</span></div><p>La racha solo cuenta cuando alcanzas tu propia meta.</p></section>
      ...
      <section className="habit-card"><h3>Límite de sesión</h3><div className="number-setting"><input aria-label="Minutos máximos por sesión" type="number" min="0" max="360" value={settings.maxSessionMinutes} onChange={e => setSettings(s => ({ ...s, maxSessionMinutes: Number(e.target.value) }))}/><span>minutos</span></div><p>Al llegar al límite, la app te propone cerrar la sesión. No bloquea el libro.</p></section>

      <section className="habit-card"><h3>Reentrada inteligente</h3><div className="number-setting"><input aria-label="Horas para ofrecer reentrada" type="number" min="1" max="720" value={settings.reentryHours} onChange={e => setSettings(s => ({ ...s, reentryHours: Number(e.target.value) }))}/><span>horas sin leer</span></div><p>Después de ese tiempo aparecerá “Recuérdame dónde estaba” con notas, preguntas, subrayados y pendientes.</p></section>
      ...
      <section className="habit-card"><h3>Mensaje</h3><textarea aria-label="Mensaje del recordatorio" rows={3} maxLength={180} value={settings.reminderText} onChange={e => setSettings(s => ({ ...s, reminderText: e.target.value }))}/><label className="habit-switch compact"><input type="checkbox" checked={settings.motivationalNudges} onChange={e => setSettings(s => ({ ...s, motivationalNudges: e.target.checked }))}/><span>Mensajes de continuidad y racha</span></label></section>

      <div className="notification-note"><strong>Android:</strong> los horarios utilizan notificaciones locales del teléfono. En web/PWA, los avisos dependen de las capacidades del navegador.</div>
      ...
      {permission && permission !== 'granted' && <p className="muted" aria-live="polite">Estado de notificaciones: {permission}</p>}
    </div>
