...
export default function TutorPanel({ open, onClose, context, onConfigureAi, onNavigateEvidence }: {
  ...
  const [messages,setMessages]=useState<TutorMessage[]>([]),[input,setInput]=useState(''),[busy,setBusy]=useState(false),[listening,setListening]=useState(false),[dictationStatus,setDictationStatus]=useState(''),[showMore,setShowMore]=useState(false),[showSelection,setShowSelection]=useState(false),[aiReady,setAiReady]=useState(false),[remaining,setRemaining]=useState(getAiEstimatedRemaining()),[responseLength,setResponseLength]=useState<AiResponseLength>(()=>getAiConfig().responseLength),[online,setOnline]=useState(()=>navigator.onLine),[operationStatus,setOperationStatus]=useState(''),[lastRequest,setLastRequest]=useState<LastRequest|null>(null)
  const recognition=useRef<SpeechRecognitionController|null>(null),endRef=useRef<HTMLDivElement|null>(null),requestAbort=useRef<AbortController|null>(null),inputRef=useRef<HTMLTextAreaElement|null>(null)

  useEffect(()=>{void db.tutorMessages.where('bookId').equals(context.bookId).sortBy('createdAt').then(rows=>setMessages(rows.slice(-60).map(r=>({...r}))).catch(()=>setMessages([]))},[context.bookId])
  useEffect(()=>{const update=()=>setRemaining(getAiEstimatedRemaining());window.addEventListener('lectoria-ai-usage',update);return()=>window.removeEventListener('lectoria-ai-usage',update)},[])
  ...
  useEffect(()=>{
    if(open){setAiReady(isAiConfigured());setRemaining(getAiEstimatedRemaining());setResponseLength(getAiConfig().responseLength);window.setTimeout(()=>endRef.current?.scrollIntoView({block:'end'}),90);return}
    recognition.current?.stop();recognition.current=null;setListening(false);stopSpeaking();requestAbort.current?.abort();requestAbort.current=null
  },[open])
  ...
  function chooseLength(length:AiResponseLength){const config=getAiConfig();saveAiConfig({...config,responseLength:length});setResponseLength(length)}
  async function safeStore(message:TutorMessage){try{return await db.tutorMessages.add({bookId:context.bookId,role:message.role,content:message.content,createdAt:message.createdAt||Date.now(),source:message.source,contextRefs:message.contextRefs})}catch{return undefined}}
  function cancelGeneration(){requestAbort.current?.abort();setOperationStatus('Consulta detenida. Puedes reformularla o volver a intentarlo.')}

  async function send(content:string,visibleLabel?:string){
    const trimmed=content.trim();if(!trimmed||busy)return
    const displayText=visibleLabel?`${visibleLabel} este fragmento`:trimmed,now=Date.now()
    const uiUserMessage:TutorMessage={role:'user',content:displayText,source:'reader',createdAt:now}
    const uiNext=[...messages,uiUserMessage],modelNext=[...messages,{role:'user' as const,content:trimmed,source:'reader' as const,createdAt:now}]
    ...
    const controller=new AbortController();requestAbort.current?.abort();requestAbort.current=controller
    try{
      ...
      const retrieval=retrievedResult.status==='fulfilled'?retrievedResult.value:{text:'',refs:[] as TutorContextRef[]},memoryText=memoryResult.status==='fulfilled'?memoryResult.value:''
      const enriched={...context,retrievedText:retrieval.text,memoryText}
      let answer:string,usedAi=false
      try{answer=await askTutor(enriched,modelNext.slice(-14),{signal:controller.signal});usedAi=true;setAiReady(true)}
      catch(error){
        if(error instanceof DOMException&&error.name==='AbortError')throw error
        const configured=isAiConfigured();setAiReady(configured);const fallback=localTutorFallback(enriched,trimmed)
      ...
      console.warn('Lectoria Tutor:',error);const message='### No se pudo completar la operación\n\nOcurrió un problema local al preparar o guardar la consulta. **Tu lectura no se ha perdido.** Inténtalo nuevamente.';setMessages([...uiNext,{role:'assistant',content:message,source:'ai',createdAt:Date.now()}])
    }finally{if(requestAbort.current===controller)requestAbort.current=null;setBusy(false)}
  }
  ...
  function toggleVoice(){
    ...
    const controller=startSpeechRecognition(text=>{setInput(v=>v?`${v.trim()} ${text}`:text);setDictationStatus('Dictado transcrito. Revísalo y envíalo cuando quieras.');window.setTimeout(()=>inputRef.current?.focus(),40)},()=>{setListening(false);recognition.current=null},message=>setDictationStatus(message))
    if(!controller){setDictationStatus('El reconocimiento de voz no está disponible.');return}recognition.current=controller;setListening(true)
  }
  async function copy(text:string){try{await navigator.clipboard.writeText(text);setOperationStatus('Respuesta copiada.')}catch{setOperationStatus('No se pudo copiar la respuesta en este dispositivo.')}}
  ...
  return <AnimatePresence>{open&&<>
    ...
    <motion.aside role="dialog" aria-modal="true" aria-label="Tutor Lectoria" tabIndex={-1} className="tutor-panel premium-tutor-panel" initial={{y:'102%',opacity:.35,scale:.985}} animate={{y:0,opacity:1,scale:1}} exit={{y:'102%',opacity:0,scale:.99}} transition={{type:'spring',stiffness:410,damping:39,mass:.82}}>
      <div className="tutor-grabber" aria-hidden="true"/><header className="tutor-header"><div><strong>Tutor Lectoria</strong><span>Texto primero · evidencia visible · sin adelantos</span></div><button onClick={onClose} aria-label="Cerrar tutor">×</button></header>
      ...
      <section className="tutor-tools" aria-label="Herramientas del tutor"><div className="response-length-control"><span>Extensión</span><button className={responseLength==='short'?'active':''} onClick={()=>chooseLength('short')}>Corta</button><button className={responseLength==='medium'?'active':''} onClick={()=>chooseLength('medium')}>Media</button><button className={responseLength==='long'?'active':''} onClick={()=>chooseLength('long')}>Larga</button></div><div className="quick-prompts primary">{primaryPrompts.map(([label,prompt])=><button key={label} onClick={()=>void send(prompt,label)} disabled={busy}>{label}</button>)}</div><button className="more-tools" onClick={()=>setShowMore(v=>!v)} aria-expanded={showMore}>{showMore?'Ocultar herramientas':'Más herramientas'}</button><AnimatePresence>{showMore&&<motion.div className="quick-prompts extras" initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}}>{extraPrompts.map(([label,prompt])=><button key={label} onClick={()=>void send(prompt,label)} disabled={busy}>{label}</button>)}</motion.div>}</AnimatePresence></section>
      <div className="chat-log" aria-live="polite">
        ...
        {messages.map((m,i)=><motion.article key={m.id??`${m.createdAt??0}-${i}`} className={`message ${m.role} source-${m.source||m.role}`} initial={{opacity:0,y:10,scale:.99}} animate={{opacity:1,y:0,scale:1}}><span className={`source-chip ${m.source||m.role}`}>{sourceLabel(m.source,m.role)}</span>{m.role==='assistant'?<TutorRichText text={m.content}/>:<p>{m.content}</p>}{m.role==='assistant'&&<><div className="message-actions"><button onClick={()=>void speak(m.content)}>Escuchar</button><button onClick={()=>void copy(m.content)}>Copiar</button></div>{m.contextRefs?.length?<div className="evidence-tray"><span>Evidencia consultada</span>{m.contextRefs.slice(0,5).map((ref,j)=><button key={`${ref.href}-${ref.progress}-${j}`} onClick={()=>onNavigateEvidence?.(ref)}><b>{ref.chapterLabel}</b><small>{Math.round(ref.progress*100)}%</small><em>{ref.excerpt.slice(0,120)}{ref.excerpt.length>120?'…':''}</em></button>)}</div>:null}</>}</motion.article>)}
      ...
      <form className="tutor-input premium-tutor-input" onSubmit={e=>{e.preventDefault();void send(input)}}><button type="button" className={listening?'mic active':'mic'} onClick={toggleVoice} aria-label={listening?'Terminar dictado':'Dictar pregunta'}>{listening?'■':'🎙'}</button><textarea ref={inputRef} rows={1} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if((e.ctrlKey||e.metaKey)&&e.key==='Enter'){e.preventDefault();void send(input)}}} placeholder="Pregunta sobre lo leído…" aria-label="Pregunta para el tutor"/><button type="submit" aria-label="Enviar" disabled={!input.trim()||busy}>↑</button></form>
    </motion.aside>
  </>}</AnimatePresence>
}
