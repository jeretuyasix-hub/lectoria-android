...
export default function Reader({ bookRecord, onBack }:{ bookRecord:BookRecord; onBack:()=>void }) {
  ...
  const [turning,setTurning]=useState<'next'|'prev'|null>(null), [limit,setLimit]=useState(false), [minutes,setMinutes]=useState(0)
  ...
  useEffect(()=>{
    ...
    async function init(){
      try{
        ...
        r.on('relocated',async(loc:any)=>{ const p=Number(loc?.start?.percentage??0); const safe=Number.isFinite(p)?Math.max(0,Math.min(1,p)):0; const cfi=loc?.start?.cfi||'', h=loc?.start?.href||'', ch=tocLabel(items,h); setProgress(safe);setHref(h);setChapter(ch);setLocation(loc?.start?.displayed?`${loc.start.displayed.page} / ${loc.start.displayed.total}`:`${Math.round(safe*100)}%`); await db.books.update(bookRecord.id,{progress:safe,cfi,lastOpenedAt:Date.now(),readingStatus:safe>.985?'read':safe>.001?'reading':bookRecord.readingStatus}); if(ch&&ch!==lastChapter.current){lastChapter.current=ch;void recordReadingEvent(bookRecord.id,'chapter','book',{chapter:ch,href:h,cfi,progress:safe})} try{const text=r.getContents()?.[0]?.document?.body?.innerText||'';setNearby(text.replace(/\s+/g,' ').trim().slice(0,6500))}catch{} })
  ...
  async function turn(dir:'next'|'prev'){ if(turning)return;setTurning(dir); try{await (dir==='next'?rendition.current?.next():rendition.current?.prev())}finally{setTimeout(()=>setTurning(null),260)} }
  ...
  return <div className={`reader-shell theme-${settings.theme}`} onClick={e=>{if(!(e.target as HTMLElement).closest('button,input,select,.side-panel,.tutor-panel,.selection-actions,.settings-sheet,.study-sheet'))setControls(v=>!v)}}>
    ...
    <div className="reader-stage"><button className="tap-zone left" onClick={e=>{e.stopPropagation();void turn('prev')}}/><div ref={host} className="epub-viewer"/><button className="tap-zone right" onClick={e=>{e.stopPropagation();void turn('next')}}/>{turning&&settings.pageMode!=='scroll'&&<motion.div className={`page-turn-overlay ${turning}`} initial={{opacity:.15,rotateY:0}} animate={{opacity:.7,rotateY:turning==='next'?-82:82}} exit={{opacity:0}} transition={{type:'spring',stiffness:260,damping:26}}><div className="curl-fold"/></motion.div>}</div>
    ...
    <NavigationPanel open={nav} onClose={()=>setNav(false)} toc={toc} bookId={bookRecord.id} progress={progress} strictSpoilers={settings.spoilerPolicy==='strict'} onNavigate={x=>void rendition.current?.display(x)}/>
