...
export function useReaderEngine(bookRecord:BookRecord,onCenterTap:()=>void,onOfferHistory:()=>void){
  ...
  function start(e:TouchEvent,doc:Document){
    if(settingsRef.current.pageMode!=='slide'||e.touches.length!==1||busy.current)return
    ...
    const p=point(e.touches[0],doc),s=size(doc),now=performance.now(),band=Math.max(24,Math.min(72,s.width*.18)),vertical=p.y>s.height*.12&&p.y<s.height*.88,edge=vertical&&(p.x<=band||p.x>=s.width-band)
  ...
  function move(e:TouchEvent){
    ...
    if(!g.dragging){if(ax<7&&ay<7)return;if(elapsed>360||ay>ax*1.18){g.blocked=true;return}if(ax<=ay)return;g.dragging=true}
    ...
    const ratio=Math.min(1,Math.abs(dx)/Math.max(1,g.width));stage.current?.style.setProperty('--lectoria-slide',String(ratio));stage.current?.classList.toggle('lectoria-slide-next',dx<0);stage.current?.classList.toggle('lectoria-slide-prev',dx>0)
  }
  ...
  function finish(e:TouchEvent,cancelled=false){
    ...
    if(!g.edge){if(distance<10&&Math.abs(dy)<10&&g.startX>g.width*.26&&g.startX<g.width*.74)onCenterTap();clearSlideVisual();return}
    if(!g.dragging){if(distance<10&&Math.abs(dy)<10)void navigatePage(g.direction);clearSlideVisual();return}
    if(e.cancelable)e.preventDefault();const correct=(g.direction==='next'&&dx<0)||(g.direction==='prev'&&dx>0),threshold=Math.min(100,g.width*.13),flick=Math.abs(g.velocityX)>.4&&distance>22
  ...
  useEffect(()=>{let dead=false;async function init(){setReady(false);setError('');try{const b=ePub(await bookRecord.file.arrayBuffer());book.current=b;await b.ready;if(dead||!host.current)return;const items=(await b.loaded.navigation)?.toc as TocItem[]||[];setToc(items);try{if(bookRecord.locations)b.locations.load(bookRecord.locations);else{await b.locations.generate(1200);await db.books.update(bookRecord.id,{locations:b.locations.save()})}}catch{}const initialFlow=settingsRef.current.pageMode==='scroll'?'scrolled-doc':'paginated';flowRef.current=initialFlow;const r=b.renderTo(host.current,{width:'100%',height:'100%',spread:'none',flow:initialFlow});rendition.current=r;r.themes.fontSize(`${settingsRef.current.fontSize}%`);r.themes.override('line-height',String(settingsRef.current.lineHeight));r.themes.override('padding',`0 ${settingsRef.current.margins}vw`);r.on('selected',(cfi:string,c:any)=>{const text=c.window.getSelection()?.toString()?.trim()||'';if(text){setSelectedCfi(cfi);setSelectedText(text)}});r.on('rendered',()=>window.setTimeout(attachFrames,0));r.on('relocated',async(loc:any)=>{const cfi=loc?.start?.cfi||'',displayed=loc?.start?.displayed;currentCfi.current=cfi;let p=Number(loc?.start?.percentage);if((!Number.isFinite(p)||p<=0)&&cfi){try{const fromLocations=Number((b.locations as any)?.percentageFromCfi?.(cfi));if(Number.isFinite(fromLocations)&&fromLocations>=0)p=fromLocations}catch{}}if((!Number.isFinite(p)||p<0)&&displayed?.total>1)p=(Math.max(1,displayed.page)-1)/Math.max(1,displayed.total-1);if(!Number.isFinite(p))p=progressRef.current||bookRecord.progress||0;const safe=Math.max(0,Math.min(1,p)),h=loc?.start?.href||'',ch=tocLabel(items,h);progressRef.current=safe;setProgress(safe);setHref(h);setChapter(ch);setLocation(displayed?`${displayed.page} / ${displayed.total}`:`${Math.round(safe*100)}%`);await db.books.update(bookRecord.id,{progress:safe,cfi,lastOpenedAt:Date.now(),readingStatus:safe>.985?'read':safe>.001?'reading':bookRecord.readingStatus});if(ch&&ch!==lastChapter.current){lastChapter.current=ch;void recordReadingEvent(bookRecord.id,'chapter','book',{chapter:ch,href:h,cfi,progress:safe})}try{const c:any=r.getContents(),active=Array.isArray(c)?c[0]:c,text=active?.document?.body?.innerText||'';setNearby(text.replace(/\s+/g,' ').trim().slice(0,6500))}catch{}window.setTimeout(attachFrames,0);window.dispatchEvent(new Event('lectoria:page-settled'))});await r.display(bookRecord.cfi||undefined);for(const h of await db.highlights.where('bookId').equals(bookRecord.id).toArray())applyHighlight(h.cfiRange,h.category,h.color||'#F5D547',h.opacity??.5);window.setTimeout(attachFrames,40);if(!dead)setReady(true)}catch(e){console.error(e);if(!dead){setError('No se pudo abrir este EPUB. Puede estar dañado o usar una estructura que esta versión todavía no interpreta.');setReady(false)}}}void init();return()=>{dead=true;try{rendition.current?.destroy()}catch{};try{book.current?.destroy()}catch{};rendition.current=null;book.current=null}},[bookRecord.id,reloadToken])
