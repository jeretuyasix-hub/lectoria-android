...
export function ReaderControls({open,onToggle,items}:{open:boolean;onToggle:()=>void;items:ControlItem[]}){
  ...
  useEffect(()=>{const clamp=()=>setPos(p=>({...p,top:Math.max(54,Math.min(window.innerHeight-62,p.top))}));clamp();window.addEventListener('resize',clamp);return()=>window.removeEventListener('resize',clamp)},[])
  ...
  function clickTrigger(ev:React.MouseEvent<HTMLButtonElement>){ev.preventDefault();ev.stopPropagation();if(open||ev.detail===0)onToggle()}
...
export function ReaderProgress({progress,chapter,location}:{progress:number;chapter:string;location:string}){return <motion.aside className="reader-progress-hud" aria-label={`Progreso de lectura: ${Math.round(progress*100)} por ciento`} initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{delay:.12,duration:.28}}><div className="reader-progress-meta"><span>{chapter||'Lectura'}</span><strong>{location||`${Math.round(progress*100)}%`}</strong><b>{Math.round(progress*100)}%</b></div><div className="reader-progress-line" aria-hidden="true"><motion.i animate={{scaleX:Math.max(.002,progress)}} transition={{type:'spring',stiffness:180,damping:28}}/></div></motion.aside>}
