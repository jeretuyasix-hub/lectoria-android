...
export default function BookOrganizePanel({ book, onClose, onSave, onDelete }: {
  ...
  useEffect(() => {
    ...
    setFavorite(Boolean(book.favorite));setSaving(false)
  }, [book])

  async function save(){if(!book||saving)return;setSaving(true);try{await onSave({ collection: collection.trim() || undefined, tags: tags.split(',').map(t => t.trim()).filter(Boolean), readingStatus: status, favorite })}finally{setSaving(false)}}

  return <AnimatePresence>{book && <motion.aside role="dialog" aria-modal="true" aria-label={`Organizar ${book.title}`} tabIndex={-1} className="organize-sheet" initial={{ y: '105%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '105%', opacity: 0 }} transition={{ type: 'spring', stiffness: 370, damping: 34 }}>
    <div className="tutor-grabber" aria-hidden="true"/>
    <header className="side-header"><div><strong>Organizar libro</strong><span>{book.title}</span></div><button onClick={onClose} aria-label="Cerrar organizador">×</button></header>
    <div className="organize-form">
      ...
      <div className="organize-actions"><button className="danger-button" disabled={saving} onClick={() => void onDelete()}>Eliminar libro</button><button className="primary-button" disabled={saving} onClick={() => void save()}>{saving?'Guardando…':'Guardar'}</button></div>
    </div>
